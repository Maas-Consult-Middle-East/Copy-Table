...
async function add_buttons() {
	// The app JavaScript is loaded for every Desk user. Only role members can
	// use the Copy Table feature.
	if (!frappe.user.has_role(COPY_TABLE_ROLE)) {
	...
	const frm = cur_frm; // nosemgrep: frappe-cur-frm-usage — no frm param available here, this runs off router change, not a form event
	...
	Object.keys(frm.fields_dict || {}).forEach((fieldname) => {
		...
		if (!field || field.df.fieldtype !== "Table" || !field.grid) {
			return;
		}
		...
		btn.on("click", async () => {
			...
			const plain_text = [headers, ...table_data].map((row) => row.join("\t")).join("\n");
...
function get_copy_value(row, df) {
	...
	const formatted = frappe.format(value, df, { only_value: true, for_print: true }, row);
